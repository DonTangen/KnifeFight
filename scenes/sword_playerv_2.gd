...
func _physics_process(delta: float) -> void:
	...
	if is_instance_valid(collision_shape):
		...
		if Input.is_action_just_pressed("jump"):
			rb.apply_central_impulse(Vector2(0, JUMP_VELOCITY))
		# Sword Swing
		if Input.is_action_just_pressed("attack-deflect"):
			self.rotation_degrees += attack_rotation + (rotation_speed/40.0 * delta)
			#collision_shape.rotation_degrees += attack_rotation + (rotation_speed/40.0 * delta)
		# Apply movement
		if direction:
			rb.apply_force(velocity, Vector2(0,0))
		else:
			rb.apply_force(-velocity, Vector2(0,0))
